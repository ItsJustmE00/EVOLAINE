...
// Servir également les images stockées dans le dossier front (project/public)
...
const allowedOrigins = [
  ...
  'https://evolaine-admin.vercel.app',
...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    if (process.env.NODE_ENV !== 'production' || origin === 'http://localhost:3005') {
      console.log('Mode développement ou accès local: origine autorisée:', origin);
      return callback(null, true);
    }
...
app.post('/api/admin/login', (req, res) => {
  ...
  // Limitation temporaire désactivée – aucune mise en quarantaine des IP pour l'instant

    // Support multiple admin comptes via env ADMIN_ACCOUNTS="user:pass,user2:pass2"
  // Utiliser uniquement les identifiants statiques souhaités
  const adminPairs = [{ u: 'EVOLAINE', p: 'TAHR1TAHR1' }];

  // DEBUG ───────────────────────────────────────────
  console.log('🛂 Liste des comptes admin autorisés:', adminPairs);
...
app.get('/api/admin/verify-token', (req, res) => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return res.status(401).json({ valid: false, error: 'Token non fourni' });
  }
  ...
  try {
    ...
    if (decoded.role !== 'admin') {
      return res.status(403).json({ valid: false, error: 'Accès non autorisé' });
    }
...
function adminAuth(req, res, next) {
  // Autoriser l'accès à la page de connexion et aux fichiers statiques
  if (req.path === '/admin/login' || 
      ...
      req.path === '/admin' ||
      req.path === '/admin/') {
    console.log('Accès autorisé à la ressource:', req.path);
    return next();
  }
...
// app.use('/api/admin', adminAuth); // désactivé temporairement pour debug
...
app.post('/api/admin/login', async (req, res) => {
  try {
    ...
    // Vérifier les identifiants (à remplacer par une vérification en base de données en production)
    if (username === 'evolaine' && password === 'tahrtahr') {
      // Créer un token JWT
...
// Servir les fichiers statiques du dossier admin avant le middleware d'authentification
...
// Route pour la racine de l'admin
app.get('/admin', (req, res) => {
  res.sendFile(path.join(adminPath, 'index.html'));
});

// Route pour toutes les autres routes de l'admin (pour le SPA)
app.get('/admin/*', (req, res) => {
  res.sendFile(path.join(adminPath, 'index.html'));
});
